...
<div class="bg-white border rounded-3xl shadow-lg sticky top-4 overflow-hidden">
    <!-- Elegant header med gradient -->
    <div class="bg-gradient-to-r from-indigo-50 to-purple-50 p-6 border-b border-gray-100 flex items-center justify-between">
        <div class="flex items-center gap-3">
            ...
            <h4 class="font-bold text-xl text-gray-900">Detaljer</h4>
        </div>
        <button class="flex items-center gap-2 px-4 py-2 text-sm text-gray-600 hover:text-gray-800 hover:bg-white/60 rounded-xl transition-all duration-200" @click="openPanel=false" wire:click="clearSelection">
    ...
    @if($selected)
        ...
        <div class="p-6 space-y-6" @if($isPublished && (!$m || $stale)) wire:poll.5s="reloadSelected" @endif>
            <!-- Status section med förbättrad design -->
            <div class="flex items-center justify-between p-4 bg-gray-50 rounded-2xl">
                <span class="text-sm font-medium text-gray-700">Status</span>
                <div class="flex items-center gap-2">
                    <svg class="w-5 h-5 {{ $badge['text'] }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        {!! $badge['svg'] !!}
                    </svg>
                    <span class="inline-flex items-center px-3 py-1.5 text-sm font-medium rounded-full {{ $badge['bg'] }} {{ $badge['text'] }}">
                        {{ $badge['label'] }}
                    </span>
                </div>
            </div>

            <!-- Titel och metadata med bättre spacing -->
            <div class="space-y-3">
                <h3 class="text-lg font-bold text-gray-900 leading-tight">{{ $selected['title'] }}</h3>
                <div class="flex flex-wrap items-center gap-3 text-sm text-gray-600">
                    ...
                    <div class="flex items-center gap-1.5">
                        ...
                        <span>{{ $selected['site'] ?: 'Ingen sajt' }}</span>
                    </div>
                    @if(!empty($selected['external_url']))
                        <a href="{{ $selected['external_url'] }}" target="_blank" class="flex items-center gap-1.5 text-indigo-600 hover:text-indigo-800 font-medium transition-colors">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <!-- Info cards med förbättrad design -->
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div class="bg-gradient-to-br from-purple-50 to-pink-50 p-4 rounded-2xl border border-purple-100">
                    ...
                    <a href="{{ route('ai.detail', $selected['ai_content_id']) }}" class="font-bold text-purple-700 hover:text-purple-900 text-lg transition-colors">
                        Öppna →
                    </a>
                </div>
            </div>
            ...
            @if($selected['status'] === 'published')
                <!-- Statistik section med förbättrad design -->
                <div class="bg-gradient-to-br from-green-50 to-emerald-50 border border-green-200 rounded-2xl p-6">
                    <div class="flex items-center justify-between mb-4">
                        <div class="flex items-center gap-2">
                            ...
                            <h5 class="text-lg font-bold text-green-800">Statistik</h5>
                        </div>
                        <button wire:click="refreshMetrics({{ (int)$selected['id'] }})" class="flex items-center gap-2 px-3 py-2 bg-white border border-green-200 text-green-700 rounded-xl hover:bg-green-50 transition-all duration-200 font-medium">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    @if($m)
                            ...
                            @foreach([
                                ...
                                <div class="bg-white p-3 rounded-xl border border-green-100">
                                    <div class="flex items-center gap-2 mb-1">
                                        ...
                                        <span class="text-xs font-medium text-green-700">{{ $metric['label'] }}</span>
                                    </div>
                                    <div class="font-bold text-gray-900 text-lg">{{ $m[$metric['key']] ?? '—' }}</div>
                                </div>
                            @endforeach
                        </div>
                        <div class="flex items-center gap-2 text-xs text-green-700 bg-green-100 px-3 py-2 rounded-lg">
                            ...
                            <span>Senast uppdaterad: {{ $selected['metrics_at'] ?: ($m['updated_at'] ?? '—') }}</span>
                        </div>
                    @else
                ...
                <div class="bg-gradient-to-r from-amber-50 to-orange-50 border-l-4 border-amber-400 p-4 rounded-r-2xl">
                    <div class="flex items-start gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-sm text-amber-700">{{ $selected['message'] }}</p>
                        </div>
                    </div>
                </div>
            @endif

            <!-- Hantera publicering section -->
            <div class="bg-gray-50 p-6 rounded-2xl space-y-4">
                ...
                <div class="space-y-3">
                    <div>
                        ...
                        <input type="datetime-local" wire:model.defer="rescheduleAt" class="w-full px-4 py-3 border border-gray-300 rounded-xl text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all" />
                        @error('rescheduleAt')
                    ...
                    <div class="flex flex-col sm:flex-row gap-3">
                        ...
                        <button @if(!$canReschedule) disabled @endif wire:click="reschedulePublication({{ (int)$selected['id'] }})"
                                class="flex-1 flex items-center justify-center gap-2 px-4 py-3 rounded-xl font-medium transition-all duration-200 {{ $canReschedule ? 'bg-indigo-600 text-white hover:bg-indigo-700 shadow-lg hover:shadow-xl transform hover:-translate-y-0.5' : 'bg-gray-200 text-gray-500 cursor-not-allowed' }}">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/>
                            </svg>
                            Ändra tid
                        ...
                        <button @if(!$canCancel) disabled @endif wire:click="cancelPublication({{ (int)$selected['id'] }})" onclick="return confirm('Avbryt denna publicering?')"
                                class="flex-1 flex items-center justify-center gap-2 px-4 py-3 rounded-xl font-medium transition-all duration-200 {{ $canCancel ? 'bg-red-600 text-white hover:bg-red-700 shadow-lg hover:shadow-xl transform hover:-translate-y-0.5' : 'bg-gray-200 text-gray-500 cursor-not-allowed' }}">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/>
                            </svg>
                            Avbryt
                    ...
                    <div class="bg-blue-50 border-l-4 border-blue-400 p-3 rounded-r-xl">
                        <p class="text-xs text-blue-700 flex items-start gap-2">
                            ...
                            <span>Ändringar av tid respekteras av köade jobb. Om en process redan körs kan publiceringen hinna gå ut.</span>
                        </p>
                    </div>
                </div>
            </div>

            <!-- Snabbplanera section -->
            <div class="bg-gradient-to-br from-green-50 to-emerald-50 p-6 rounded-2xl space-y-4 border border-green-200">
                <h5 class="text-lg font-bold text-green-800 flex items-center gap-2">
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <select wire:model.defer="quickContentId" class="w-full px-4 py-3 border border-green-300 rounded-xl text-sm focus:ring-2 focus:ring-green-500 focus:border-green-500 bg-white">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            ...
                            <select wire:model.defer="quickTarget" class="w-full px-4 py-3 border border-green-300 rounded-xl text-sm focus:ring-2 focus:ring-green-500 focus:border-green-500 bg-white">
                        ...
                        <div>
                            ...
                            <input type="datetime-local" wire:model.defer="quickScheduleAt" class="w-full px-4 py-3 border border-green-300 rounded-xl text-sm focus:ring-2 focus:ring-green-500 focus:border-green-500 bg-white" />
                    ...
                    <div class="flex justify-end">
                        <button wire:click="createQuickPublication" class="flex items-center gap-2 px-6 py-3 bg-emerald-600 text-white rounded-xl hover:bg-emerald-700 font-semibold shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 transition-all duration-200">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div class="bg-emerald-100 p-3 rounded-xl border border-emerald-200">
                        <p class="text-xs text-emerald-700 flex items-start gap-2">
                            ...
                            <span>Använd AI-texten som grund. Bilder och finjustering kan läggas till i detaljsidan.</span>
                        </p>
                    </div>
                </div>
            </div>
