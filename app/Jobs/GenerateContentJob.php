...
{$templateHint}
    ...
    private function buildBulkPrompt(array $inputs, string $language): string
    {
        ...
        if ($language === 'en') {
            return "YOUR TASK:\n{$instruction}\n\nKEY VARIABLES THAT MUST APPEAR IN THE TEXT:\n{$varsString}\n\nCRITICAL REQUIREMENTS FOR NATURAL, HIGH-QUALITY TEXT:\n\n1. VARIABLE USAGE (SEO-CRITICAL):\n- Use each variable value (like '{$varsString}') 5-8 times distributed naturally throughout\n- Vary the context: in headings, body paragraphs, lists, and conclusions\n- Use synonyms and variations around the keywords\n- Example: if stad=Stockholm, also mention \"in the capital\", \"locally in Stockholm\", \"Stockholm residents\"\n\n2. AUTHENTICITY & ANTI-AI DETECTION:\n- Write with personality and human touch\n- Use varied sentence structures (short, medium, long)\n- Include occasional rhetorical questions\n- Add specific details and concrete examples\n- Avoid generic phrases like \"we offer\", \"high quality\", \"best choice\"\n- Use contractions and natural language\n- Vary paragraph length (some 2 sentences, some 4-5)\n\n3. FACTUAL ACCURACY:\n- DO NOT invent services, events, workshops, physical stores, or claims not in the instruction\n- Only state what's explicitly mentioned\n- If uncertain, focus on general benefits without false specifics\n\n4. STRUCTURE FOR ENGAGEMENT:\n- Start with a hook (question, surprising fact, or bold statement)\n- Use H2/H3 headings naturally (not formulaic)\n- Include specific examples and scenarios\n- End with a clear, compelling call-to-action\n\n5. DELIVERY FORMAT:\n- First line: SEO-optimized title (max 60 chars)\n- Blank line\n- Then: natural, flowing content\n- Do NOT include the title in the body text";
        }

        if ($language === 'de') {
            return "DEINE AUFGABE:\n{$instruction}\n\nSCHLÜSSELVARIABLEN DIE IM TEXT ERSCHEINEN MÜSSEN:\n{$varsString}\n\nKRITISCHE ANFORDERUNGEN FÜR NATÜRLICHEN, HOCHWERTIGEN TEXT:\n\n1. VARIABLEN-NUTZUNG (SEO-KRITISCH):\n- Verwende jeden Variablenwert (wie '{$varsString}') 5-8 mal natürlich verteilt\n- Variiere den Kontext: in Überschriften, Absätzen, Listen und Schluss\n- Nutze Synonyme und Variationen um die Keywords\n- Beispiel: wenn stad=Stockholm, erwähne auch \"in der Hauptstadt\", \"lokal in Stockholm\", \"Stockholmer\"\n\n2. AUTHENTIZITÄT & ANTI-KI-ERKENNUNG:\n- Schreibe mit Persönlichkeit und menschlicher Note\n- Nutze unterschiedliche Satzstrukturen (kurz, mittel, lang)\n- Füge gelegentlich rhetorische Fragen ein\n- Füge spezifische Details und konkrete Beispiele hinzu\n- Vermeide generische Phrasen wie \"wir bieten\", \"hohe Qualität\", \"beste Wahl\"\n- Nutze Kontraktionen und natürliche Sprache\n- Variiere Absatzlängen (manche 2 Sätze, manche 4-5)\n\n3. FAKTISCHE GENAUIGKEIT:\n- Erfinde KEINE Dienstleistungen, Events, Workshops, Läden oder Behauptungen die nicht in der Anweisung stehen\n- Nenne nur was explizit erwähnt wird\n- Bei Unsicherheit: fokussiere auf allgemeine Vorteile ohne falsche Details\n\n4. STRUKTUR FÜR ENGAGEMENT:\n- Starte mit Hook (Frage, überraschende Tatsache oder kühne Aussage)\n- Nutze H2/H3 Überschriften natürlich (nicht formelhaft)\n- Füge spezifische Beispiele und Szenarien ein\n- Ende mit klarem, überzeugendem Call-to-Action\n\n5. LIEFERFORMAT:\n- Erste Zeile: SEO-optimierter Titel (max 60 Zeichen)\n- Leerzeile\n- Dann: natürlicher, fließender Inhalt\n- Füge den Titel NICHT in den Fließtext ein";
        }

        return "DITT UPPDRAG:\n{$instruction}\n\nNYCKELVARIABLER SOM MÅSTE FÖREKOMMA I TEXTEN:\n{$varsString}\n\nKRITISKA KRAV FÖR NATURLIG, HÖGKVALITATIV TEXT:\n\n1. VARIABELANVÄNDNING (SEO-KRITISKT):\n- Använd varje variabelvärde (som '{$varsString}') 5-8 gånger naturligt fördelat\n- Variera kontexten: i rubriker, stycken, listor och avslutning\n- Använd synonymer och variationer kring nyckelorden\n- Exempel: om stad=Stockholm, nämn även \"i huvudstaden\", \"lokalt i Stockholm\", \"stockholmare\"\n\n2. AUTENTICITET & ANTI-AI-DETEKTERING:\n- Skriv med personlighet och mänsklig touch\n- Använd varierade meningsstrukturer (korta, medellånga, långa)\n- Inkludera enstaka retoriska frågor\n- Lägg till specifika detaljer och konkreta exempel\n- Undvik generiska fraser som \"vi erbjuder\", \"hög kvalitet\", \"bästa valet\"\n- Använd sammandragningar och naturligt språk\n- Variera styckelängder (vissa 2 meningar, vissa 4-5)\n\n3. FAKTISK NOGGRANNHET:\n- Hitta INTE på tjänster, evenemang, workshops, fysiska butiker eller påståenden som inte finns i instruktionen\n- Ange endast vad som uttryckligen nämns\n- Vid osäkerhet: fokusera på generella fördelar utan falska detaljer\n\n4. STRUKTUR FÖR ENGAGEMANG:\n- Börja med en hook (fråga, överraskande fakta eller djärvt påstående)\n- Använd H2/H3-rubriker naturligt (inte formelbaserat)\n- Inkludera specifika exempel och scenarion\n- Avsluta med tydlig, övertygande uppmaning till handling\n\n5. LEVERANSFORMAT:\n- Första raden: SEO-optimerad titel (max 60 tecken)\n- Blank rad\n- Sedan: naturligt, flytande innehåll\n- Inkludera INTE titeln i brödtexten";
    }
    ...
    private function buildHardRules(AiContent $content, array $inputs, string $language): string
    {
        ...
        */
        ...
        if (!empty($inputs['bulkVariables'])) {
            ...
            $rules[] = match($language) {
                'en' => "MANDATORY: The exact terms {$valueList} MUST each appear 5-8 times naturally distributed. Integrate in headings, body, examples, and conclusion. Use variations and synonyms around these terms.",
                'de' => "OBLIGATORISCH: Die Begriffe {$valueList} MÜSSEN jeweils 5-8 mal natürlich vorkommen. Integriere in Überschriften, Text, Beispielen und Schluss. Nutze Variationen und Synonyme.",
                default => "OBLIGATORISKT: Termerna {$valueList} MÅSTE vardera förekomma 5-8 gånger naturligt fördelade. Integrera i rubriker, text, exempel och avslutning. Använd variationer och synonymer.",
            };

            $rules[] = match($language) {
                'en' => "Write like a human, not AI: vary sentence length, use personality, ask questions, give concrete examples. Avoid formulaic structures.",
                'de' => "Schreibe wie ein Mensch, nicht KI: variiere Satzlänge, nutze Persönlichkeit, stelle Fragen, gib konkrete Beispiele. Vermeide Formeln.",
                default => "Skriv som en människa, inte AI: variera meningslängd, använd personlighet, ställ frågor, ge konkreta exempel. Undvik formler.",
            };

            $rules[] = match($language) {
                'en' => "DO NOT invent: services, events, workshops, stores, testimonials, or specific claims not in the instruction. Stay factual.",
                'de' => "Erfinde NICHT: Dienstleistungen, Events, Workshops, Läden, Testimonials oder spezifische Behauptungen die nicht in der Anweisung sind. Bleib faktisch.",
                default => "Hitta INTE på: tjänster, evenemang, workshops, butiker, testimonials eller specifika påståenden som inte finns i instruktionen. Håll dig till fakta.",
            };
        }
